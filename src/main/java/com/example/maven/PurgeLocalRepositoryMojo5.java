...
public class PurgeLocalRepositoryMojo5 extends AbstractMojo {
    ...
    /**
     * Löscht die gesammelten Pfade über einen separaten Betriebssystem-Prozess.
     * So können auch von der aktuellen JVM gesperrte Dateien entfernt werden.
     * Unterstützt Windows ({@code cmd /c}) und Unix ({@code sh -c}).
     */
    private void deleteInSeparateProcess(List<Path> paths) throws MojoExecutionException {
        ...
        for (Path path : paths) {
            ...
            getLog().info("  Lösche: " + absolute);

            if (windows) {
                commands.append(Files.isDirectory(path)
                        ? "rmdir /s /q \"" + absolute + "\" & "
                        : "del /f /q \"" + absolute + "\" & ");
            } else {
                commands.append(Files.isDirectory(path)
        ...
        String[] command = windows
                ? new String[]{"cmd", "/c", commands.toString()}
                : new String[]{"sh", "-c", commands.toString()};
        ...
        try {
            ...
            if (exitCode != 0) {
                throw new MojoExecutionException("Lösch-Prozess beendet mit Exit-Code " + exitCode);
            }
